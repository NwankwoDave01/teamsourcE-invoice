...
import { useIntegrationHealth } from "@/hooks/useCompanyData";
    ...
    <div>
      ...
      <div className="grid gap-4 p-6 md:grid-cols-2 xl:grid-cols-3">
        {isLoading ? (
          ...
          <p className="col-span-full py-12 text-center text-sm text-muted-foreground">No integrations configured.</p>
        ) : (
          integrations.map((i) => (
            <Card key={i.name} className="p-5 shadow-elegant-sm">
              <div className="flex items-start justify-between">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">Last incident: {i.last_incident ?? "—"}</p>
                </div>
